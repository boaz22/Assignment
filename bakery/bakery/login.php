...
$username_err =$choice_err= $password_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["password"]))){
     ...
     if(empty(trim($_POST["choice"]))){
        $choice_err = "Please enter your choice.";
    } else{
        $choice = trim($_POST["choice"]);
    }
    
    // Validate credentials
    if(empty($username_err) && empty($password_err)&& empty($choice_err)){
        // Prepare a select statement
        $sq = "SELECT id, username, `password`,choice FROM users WHERE username = ?";
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                    	...
                    	if ($_POST['password'] === $password && $_POST['choice'] === 'Admin') {
                            ...
                            header("location: welcome.php");
                        }else if($_POST['password'] === $password && $_POST['choice'] === 'Client'){
                                  // Password is correct, so start a new session
                            session_start();
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <p>Welcome to <a href="index.php">Apple Safaris</a></p>
        </form>
    </div>    
</body>
</html>